...
public class Map3View implements Initializable {
    ...
    private void addRaiderToMap(int i) {
        ...
        int randomPoint = random.nextInt(2)+1;

        PointF[] point;
        if(randomPoint == 2)
            point = map.getPoints2();
        else
            point = map.getPoints();
        ...
        switch (randomNumber) {
            case 1:
                raider = new GhostRaider(point);
                break;
            case 2:
                raider = new DarkknightRaider(point);
                break;
            case 3:
                raider = new GoblinRaider(point);
                break;
            default:
                raider = new ZombieRaider(point);
                break;
        }
            ...
            if (allDead & count) {
                if(wave==2){
                    ...
                    if(PlayerController.getPlayerController().getPlayer().getLevel()<4)
                        PlayerController.getPlayerController().getPlayer().setLevel(4);
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        map = new MapGame(slots, points, 2, 2000, points2);
